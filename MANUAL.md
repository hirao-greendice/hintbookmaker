...
Apps Script 経由で読み込むと、スプレッドシートの `body` セルに設定した左揃え・中央揃え・右揃えが本文全体に反映されます。配置が指定されていない場合は左揃えです。

本文の一部分だけ配置を変える場合は、次のコマンドで囲みます。

```text
[[left]]ここだけ左揃え[[/left]]
[[center]]ここだけ中央揃え[[/center]]
[[right]]ここだけ右揃え[[/right]]
```

コマンドで囲んだ部分は、セル全体の配置より優先されます。外側の文章はセルの配置を引き継ぎます。`[[box]]` や `[[hl]]` とも併用できます。閉じ忘れや組み合わせが間違った配置コマンドは、文字として表示されます。
